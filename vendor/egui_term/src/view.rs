...
impl<'a> TerminalView<'a> {
    ...
    fn process_input(
        ...
        if self.has_focus {
            ...
            let cursor_min = Pos2::new(
                layout.rect.min.x + cell_width * point.column.0 as f32,
                layout.rect.min.y + cell_height * line_num as f32,
            );
            let cursor_rect = Rect::from_min_size(
                cursor_min,
                Vec2::new(cell_width.max(1.0), cell_height.max(1.0)),
            );
...
fn process_keyboard_event(
    ...
    match event {
        ...
        egui::Event::Ime(ime) => match ime {
            egui::ImeEvent::Commit(text) => InputAction::BackendCall(
                BackendCommand::Write(text.as_bytes().to_vec()),
            ),
            egui::ImeEvent::Enabled
            | egui::ImeEvent::Preedit(_)
            | egui::ImeEvent::Disabled => InputAction::Ignore,
        },
