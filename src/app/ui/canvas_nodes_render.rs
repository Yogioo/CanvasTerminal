...
impl GraphApp {
    pub(in crate::app::ui) fn draw_resize_handle(painter: &Painter, node_rect: Rect, zoom_scale: f32, color: Color32) {
    ...
    pub(in crate::app::ui) fn draw_nodes(
        ...
        let mut render_nodes = self.ws.nodes.clone();
        ...
        for node in &render_nodes {
            ...
            let is_selected = self.ws.selected_nodes.contains(&node.id);
            ...
            match node.kind {
                NodeKind::Terminal => {
                    ...
                    if !is_title_editing {
                        let state_text = if self.ws.terminal_backends.contains_key(&node.id) {
                            "状态: Running"
                        } else if self.ws.terminal_exited.contains(&node.id) {
                            "状态: Exited"
                        } else {
                            "状态: Starting"
                        };
                    ...
                    let hide_terminal_for_zoom = zoom_scale < self.ws.terminal_hide_zoom_threshold
                        && self.ws.editing_startup_node != Some(node.id)
                        && self.ws.editing_working_directory_node != Some(node.id);

                    if let Some(term_rect) = self.terminal_content_rect_screen(node.id, rect) {
                        if !hide_terminal_for_zoom {
                            if self.ws.editing_startup_node == Some(node.id)
                                || self.ws.editing_working_directory_node == Some(node.id)
                            {
                                ...
                                if self.ws.editing_working_directory_node == Some(node.id) {
                ...
                NodeKind::Text => {
                    let hide_text_for_zoom = zoom_scale < self.ws.text_hide_zoom_threshold;
                    let is_editing = self.ws.editing_text_node == Some(node.id);
                ...
                NodeKind::Decision => {
                    ...
                    let is_decision_editing = self.ws.editing_decision_buttons_node == Some(node.id);
    ...
    pub(in crate::app::ui) fn script_color_from_lua(value: &str) -> Option<Color32> {
    ...
    pub(in crate::app::ui) fn ensure_script_bg_texture(
        &mut self,
        node_id: usize,
        image_path: &str,
        ctx: &egui::Context,
        ...
        if let Some(texture) = self.ws.script_bg_textures.get(&key) {
            return Some(texture.id());
        }
        if self.ws.script_bg_texture_errors.contains_key(&key) {
            return None;
        }
        ...
        match Self::load_image_from_path(image_path) {
            Ok(color_image) => {
                ...
                let id = texture.id();
                self.ws.script_bg_textures.insert(key, texture);
                Some(id)
            }
            Err(err) => {
                self.ws.script_bg_texture_errors.insert(key, err);
                None
            }
        }
    }
