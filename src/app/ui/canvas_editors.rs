...
impl GraphApp {
    ...
    pub(in crate::app::ui) fn handle_script_queue_editor(&mut self, ctx: &egui::Context) {
        ...
        window.show(ctx, |ui| {
            ...
            if self.pending_script_queue_focus == Some(node_id) {
                ctx.memory_mut(|m| m.request_focus(editor_id));
            }
            ...
            let edit_response = ui.add_sized(
                ...
                egui::TextEdit::multiline(&mut self.script_queue_edit_buffer)
                    .id(editor_id)
                    .desired_width(f32::INFINITY)
                    .background_color(Color32::from_rgb(14, 18, 30))
                    .text_color(Color32::WHITE)
    ...
    pub(in crate::app::ui) fn handle_script_buttons_editor(&mut self, ctx: &egui::Context) {
        ...
        let screen_rect = ctx.screen_rect();
        egui::Area::new(egui::Id::new(("script-buttons-modal-mask", node_id)))
            .order(egui::Order::Foreground)
            .interactable(false)
            .fixed_pos(screen_rect.min)
            .show(ctx, |ui| {
        ...
        let modal_size = vec2(480.0, 260.0);
        let modal_pos = screen_rect.center() - modal_size * 0.5;

        let mut open = true;
        let window = egui::Window::new(
            egui::RichText::new(format!("{node_title} · 按钮配置"))
                .color(Color32::WHITE)
                .strong(),
        )
        .open(&mut open)
        ...
        .default_size(modal_size)
        .default_pos(modal_pos)
        .min_size(vec2(360.0, 180.0))
        .max_size(vec2(600.0, 400.0))
        .frame(
            egui::Frame::new()
                .fill(Color32::from_rgb(25, 29, 42))
                .stroke(egui::Stroke::new(1.0, Color32::from_rgb(86, 102, 138)))
        ...
        window.show(ctx, |ui| {
            ui.visuals_mut().override_text_color = Some(Color32::WHITE);
            ui.label("每行配置一个按钮：名称 | 事件名 (route_key) | 颜色");
            ui.add_space(8.0);

            egui::ScrollArea::vertical()
                .id_salt(("script-buttons-editor-scroll", node_id))
                .auto_shrink([false, true])
                .max_height(280.0)
                .show(ui, |ui| {
                    let mut remove_row: Option<usize> = None;

                    for (row_idx, row) in self.script_buttons_edit_rows.iter_mut().enumerate() {
                        ui.horizontal(|ui| {
                            let row_style = ui.style_mut();
                            row_style.visuals.override_text_color = Some(Color32::BLACK);
                            row_style.visuals.widgets.inactive.bg_fill =
                                Color32::from_rgb(245, 248, 252);
                            row_style.visuals.widgets.hovered.bg_fill =
                                Color32::from_rgb(236, 242, 250);
                            row_style.visuals.widgets.active.bg_fill = Color32::from_rgb(226, 236, 248);
                            row_style.visuals.widgets.inactive.fg_stroke.color = Color32::BLACK;
                            row_style.visuals.widgets.hovered.fg_stroke.color = Color32::BLACK;
                            row_style.visuals.widgets.active.fg_stroke.color = Color32::BLACK;
                            ...
                            let _ = ui.add_sized(
                                ...
                                TextEdit::singleline(&mut row.event_key)
                                    .text_color(Color32::BLACK)
                                    .background_color(Color32::from_rgb(248, 251, 255))
                                    .hint_text("事件名 route_key"),
                            );

                            let color_btn = egui::Button::new(egui::RichText::new("  ")
                                    .color(Color32::from_rgb(
                                        row.color_rgb[0], row.color_rgb[1], row.color_rgb[2],
                                    )))
                                .min_size(vec2(30.0, 22.0))
                                .fill(Color32::from_rgb(
                                    row.color_rgb[0], row.color_rgb[1], row.color_rgb[2],
                                ))
                                .stroke(egui::Stroke::new(1.0, Color32::from_rgb(90, 90, 90)));
                            let _ = ui.add(color_btn);
                        });
                        ui.add_space(4.0);
                    }

                    if let Some(row_idx) = remove_row {
                        self.remove_script_button_row(row_idx);
                    }

                    let add_btn =
                        egui::Button::new(egui::RichText::new("+ 新增按钮").color(Color32::BLACK))
                            .fill(Color32::from_rgb(228, 236, 246))
                            .stroke(egui::Stroke::new(1.0, Color32::from_rgb(150, 166, 188)));
                    if ui.add(add_btn).clicked() {
                        self.add_script_button_row();
                    }
                });

            if let Some(err) = &self.script_buttons_edit_error {
                ui.colored_label(Color32::from_rgb(255, 120, 120), err);
            }
        ...
        if save_clicked {
            self.commit_script_buttons_edit();
        } else if cancel_clicked {
            self.cancel_script_buttons_edit();
        }
    }
